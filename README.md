...
1. **Crawling & ingestion**  
   - Use `notebooks/crawler.ipynb` (or similar scripts) to fetch messages from a Telegram dataset.  
   - The crawler writes `graph.json` and `message_nodes.csv` under `notebooks/data/<dataset>`, providing the raw network and message data that the topic modelling notebook consumes.  
2. **Topic modelling**  
