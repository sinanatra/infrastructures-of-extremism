import{f as S,a as U}from"../chunks/DdPZTTEu.js";import{W as q,Y as $,Z as k,i as u,au as V,_ as p,at as M,a0 as K,$ as Z,X as O}from"../chunks/C8ib__8j.js";import{s as Y}from"../chunks/BgYwbzME.js";import{e as J}from"../chunks/DJ2x-88Q.js";import{p as Q,b as ee,i as j}from"../chunks/ZeJlZOjy.js";import{s as te}from"../chunks/m2wJRAEl.js";import{o as ae}from"../chunks/ChNsLLqb.js";const re=async({fetch:T})=>{const[g,s]=await Promise.all([T("/data/datasets.json"),T("/data/dataset-themes.json")]);let c=[];if(g.ok)try{c=await g.json()}catch(t){console.warn("Failed to parse datasets.json",t)}Array.isArray(c)||(c=[]);let n=[];if(s.ok)try{n=await s.json()}catch(t){console.warn("Failed to parse dataset-themes.json",t)}Array.isArray(n)||(n=[]);const e=new Map(n.filter(t=>t?.slug&&t.theme).map(t=>[t.slug,t.theme]));return{datasets:c.map(t=>({...t,theme:e.get(t?.slug)??t?.theme}))}},_e=Object.freeze(Object.defineProperty({__proto__:null,load:re},Symbol.toStringTag,{value:"Module"})),ne=`attribute vec2 a_pos;
varying vec2 v_uv;
...
`;var oe=S('<canvas class="block w-full h-full"></canvas>');function se(T,g){q(g,!0);let s=Q(g,"images",19,()=>[]),c=0,n,e,a,t=null,f=null,w=null,L,z,r,h,A,E,v=[1,1],x=[1,1],F=[1,1],b=!1,_=!1,D=0,G=-1,R;const X=4e3,N=100;function I(){if(e=n.getContext("webgl"),!e)return!1;const i=(l,y)=>{const B=e.createShader(l);return e.shaderSource(B,y),e.compileShader(B),e.getShaderParameter(B,e.COMPILE_STATUS)||console.error(e.getShaderInfoLog(B)),B};a=e.createProgram(),e.attachShader(a,i(e.VERTEX_SHADER,ne)),e.attachShader(a,i(e.FRAGMENT_SHADER,ie)),e.linkProgram(a),e.useProgram(a);const o=e.createBuffer();e.bindBuffer(e.ARRAY_BUFFER,o),e.bufferData(e.ARRAY_BUFFER,new Float32Array([-1,-1,1,-1,-1,1,1,1]),e.STATIC_DRAW);const m=e.getAttribLocation(a,"a_pos");return e.enableVertexAttribArray(m),e.vertexAttribPointer(m,2,e.FLOAT,!1,0,0),L=e.getUniformLocation(a,"u_t"),z=e.getUniformLocation(a,"u_sizeA"),r=e.getUniformLocation(a,"u_sizeB"),h=e.getUniformLocation(a,"u_res"),A=e.getUniformLocation(a,"u_dpr"),E=e.getUniformLocation(a,"u_noiseSize"),e.uniform1i(e.getUniformLocation(a,"u_texA"),0),e.uniform1i(e.getUniformLocation(a,"u_texB"),1),e.uniform1i(e.getUniformLocation(a,"u_noise"),2),!0}function P(i,o=!1){e.pixelStorei(e.UNPACK_FLIP_Y_WEBGL,!o);const m=e.createTexture();e.bindTexture(e.TEXTURE_2D,m);const l=o?e.REPEAT:e.CLAMP_TO_EDGE;return e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_S,l),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_T,l),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MIN_FILTER,e.LINEAR),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MAG_FILTER,e.LINEAR),e.texImage2D(e.TEXTURE_2D,0,e.RGBA,e.RGBA,e.UNSIGNED_BYTE,i),m}function d(i){return new Promise((o,m)=>{const l=new Image;l.onload=()=>o(l),l.onerror=m,l.src=i})}function C(i){if(R=requestAnimationFrame(C),!t||!w||!e)return;const o=n?.clientWidth*devicePixelRatio|0,m=n?.clientHeight*devicePixelRatio|0;(n.width!==o||n.height!==m)&&(n.width=o,n.height=m,e.viewport(0,0,o,m));let l=1,y=b&&f!==null;y&&(l=(i-D)/X,l>=1&&(b=!1,y=!1,e.deleteTexture(t),t=f,f=null,v=[...x],c=G,l=1,setTimeout(()=>W((c+1)%s().length),N))),e.uniform2f(h,n.width,n.height),e.uniform1f(A,devicePixelRatio),e.uniform2fv(E,F),e.uniform1f(L,y?l:1),e.uniform2fv(z,v),e.uniform2fv(r,y?x:v),e.activeTexture(e.TEXTURE0),e.bindTexture(e.TEXTURE_2D,t),e.activeTexture(e.TEXTURE1),e.bindTexture(e.TEXTURE_2D,y?f:t),e.activeTexture(e.TEXTURE2),e.bindTexture(e.TEXTURE_2D,w),e.drawArrays(e.TRIANGLE_STRIP,0,4)}async function W(i){if(!(_||b)){_=!0,G=i;try{const o=await d(s()[i]);if(!e)return;f&&e.deleteTexture(f),f=P(o),x=[o.naturalWidth,o.naturalHeight],b=!0,D=performance.now()}catch{setTimeout(()=>W((i+1)%s().length),N)}finally{_=!1}}}ae(async()=>{if(!s().length||!I())return;c=Math.floor(Math.random()*s().length);const[i,o]=await Promise.all([d("/pattern.png"),d(s()[c])]);return w=P(i,!0),F=[i.naturalWidth,i.naturalHeight],t=P(o),v=[o.naturalWidth,o.naturalHeight],R=requestAnimationFrame(C),W((c+1)%s().length),()=>cancelAnimationFrame(R)});var H=oe();ee(H,i=>n=i,()=>n),U(T,H),$()}var le=S("<span>/</span>"),ce=S('<!> <a class="hover:underline" data-sveltekit-reload=""> </a>',1),me=S('<div class="border-b border-gray-300 py-4 text-black"><div> </div> <div class="flex gap-2 mt-1.5 text-xs tracking-wide"></div></div>'),ue=S('<div class="mb-12 text-base tabular-nums"></div>'),fe=S(`<section class="bg-white text-gray-600 flex flex-col md:h-screen md:flex-row md:overflow-hidden"><div class="w-full h-[45vh] md:h-full md:flex-1 overflow-hidden"><!></div> <article class="w-full md:h-full md:w-[360px] md:flex-shrink-0 md:overflow-y-auto bg-[#fefefe] text-black px-4 pt-4 pb-20 text-base leading-relaxed"><h1 class="text-xl mb-4 max-w-[300px] text-black">Infrastructures of Extremism</h1> <p class="mb-3">Right-wing extremist organisations have gained visibility on European
      ...
      borders.</p> <h2 class="text-base border-t border-gray-300 mt-20 mb-10 pt-2">Several Telegram groups have been examined</h2> <!> <p class="text-xs border-t border-gray-300 mt-20 pt-2">Infrastructures of Extremism is a project by <a class="underline" href="https://giacomo.website/" target="_blank" rel="noopener noreferrer">Giacomo Nanni</a> in collaboration with metaLAB (at) Harvard & Berlin.</p></article></section>`);function ye(T,g){q(g,!0);const s=V(()=>(g.data?.datasets??[]).slice().sort((r,h)=>(h?.postCount??0)-(r?.postCount??0))),c=V(()=>u(s).map(r=>`/cover/${r.slug}.png`)),n=[{key:"graph",label:"Time",href:r=>`/${encodeURIComponent(r)}`},{key:"pie",label:"Topics",href:r=>`/${encodeURIComponent(r)}/pie`},{key:"tree",label:"Network",href:r=>`/${encodeURIComponent(r)}/tree`}];var e=fe(),a=k(e),t=k(a);{var f=r=>{se(r,{get images(){return u(c)}})};j(t,r=>{u(c).length&&r(f)})}p(a);var w=M(a,2),L=M(k(w),28);{var z=r=>{var h=ue();J(h,21,()=>u(s),A=>A.slug,(A,E)=>{var v=me(),x=k(v),F=k(x,!0);p(x);var b=M(x,2);J(b,23,()=>n,_=>_.key,(_,D,G)=>{var R=ce(),X=Z(R);{var N=d=>{var C=le();U(d,C)};j(X,d=>{u(G)>0&&d(N)})}var I=M(X,2),P=k(I,!0);p(I),O(d=>{te(I,"href",d),Y(P,u(D).label)},[()=>u(D).href(u(E).slug)]),U(_,R)}),p(b),p(v),O(()=>Y(F,u(E).label||u(E).slug)),U(A,v)}),p(h),U(r,h)};j(L,r=>{u(s).length&&r(z)})}K(2),p(w),p(e),U(T,e),$()}export{ye as component,_e as universal};
